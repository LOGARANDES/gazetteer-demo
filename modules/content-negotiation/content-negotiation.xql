...
let $data :=
    if(request:get-parameter('id', '') != '' or request:get-parameter('doc', '') != '') then
        data:get-rec(request:get-parameter('id', ''))
    else if(request:get-parameter-names() != '') then 
        let $search := data:search('') 
        let $hits := if(count($search) gt 1) then $search else collection($global:data-root)
        return $hits
        (:
                    ...
                    <results>{
                        ...
                        return 
                            <json:value json:array="true">
                                <id>{$id}</id>
                                {$title}
                                <hits>{normalize-space(string-join((tei2html:output-kwic($expanded)),' '))}</hits>
                            </json:value>
                        }
                    </results>
                </root>
