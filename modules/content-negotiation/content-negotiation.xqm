...
declare function cntneg:content-negotiation($data as item()*, $content-type as xs:string?, $path as xs:string?){
    ...
    return 
        if($flag = 'atom') then 
            ...
            (response:set-header("Content-Type", "text/csv; charset=utf-8"), tei2csv:tei2csv($data))  
        ...
        else if($flag = ('turtle','ttl')) then <div>Unsupported</div>
        ...
        else if($flag = ('txt','text')) then <div>Unsupported</div>
...
declare function cntneg:content-negotiation-restxq($data as item()*, $content-type as xs:string?, $path as xs:string?){
    ...
    return 
        ...
        else if($flag = 'txt') then <message>Not an available data format.</message>
           (: (<rest:response> 
                <http:response status="200"> 
