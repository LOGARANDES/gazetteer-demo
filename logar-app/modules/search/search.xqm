...
declare %templates:wrap function search:get-results($node as node(), $model as map(*), $collection as xs:string?){
    ...
    let $eval-string := 
                        if($coll = 'places') then concat(places:query-string(),facets:facet-filter())
                        else concat(search:query-string($collection),facets:facet-filter())
    return                         
    map {"hits" := 
...
if(exists(request:get-parameter-names())) then 
    <div>
     <h4>Browse by</h4>
     {
        ...
        let $facets := $facet-nodes//tei:location[@type='ancient']
...
declare %templates:wrap function search:build-page($node as node()*, $model as map(*), $collection as xs:string?) {
    if(exists(request:get-parameter-names())) then 
        <div class="row">
            <div class="col-md-3">{search:facets($node, $model)}</div>
            <div class="col-md-9">{search:show-hits($node, $model, $collection)}</div>
        </div>
    else ()
};
