...
declare variable $global:data-root := 
    let $app-root := $global:get-repo//repo:app-root/text()  
    let $data-root := concat($global:get-repo//repo:data-root/text(),'/data') 
    return
...
declare variable $global:nav-base := 
    if($global:get-repo//repo:nav-base/text() != '') then $global:get-repo//repo:nav-base/text()
    (: For app set to root '/' see syriaca.org production site. :)
    else if($global:get-repo//repo:nav-base/text() = '/') then ''
    else '';

(: Base URI used in record tei:idno :)
declare variable $global:base-uri := $global:get-repo//repo:base_uri/text();

declare variable $global:app-title := $global:get-repo//repo:title/text();

declare variable $global:app-url := $global:get-repo//repo:url/text();

declare variable $global:id-path := $global:get-repo//repo:id-path/text();

(: Map rendering, google or leaflet :)
declare variable $global:app-map-option := $global:get-repo//repo:maps/repo:option[@selected='true']/text();

(: Map rendering, google or leaflet :)
declare variable $global:map-api-key := $global:get-repo//repo:maps/repo:option[@selected='true']/@api-key;

(: Recaptcha Key, Store as environemnt variable. :)
declare variable $global:recaptcha := 
    if($global:get-config//recaptcha/recaptcha-key-variable/text() != '') then
        environment-variable($global:get-config//recaptcha/recaptcha-key-variable/text())
    else if($global:get-config//recaptcha/recaptcha-key/text() != '') then
        $global:get-config//recaptcha/recaptcha-key/text()    
    else ();
...
declare function global:collection-vars($collection as xs:string?) as node()?{
    let $collection-config := $global:get-repo//repo:collections
...
declare function global:odd2text($element as xs:string?, $label as xs:string?) as xs:string* {
    let $odd-path := $global:get-repo//repo:odd/text()
    let $odd-file := 
...
declare function global:keyboard-select-menu($input-id as xs:string){
    ...
    if($global:get-repo//repo:keyboard-options/child::*) then 
        <ul xmlns="http://www.w3.org/1999/xhtml" class="dropdown-menu" test="TEST">
            {
            for $layout in $global:get-repo//repo:keyboard-options/repo:option
            return  
