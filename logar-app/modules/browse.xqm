...
:)
declare function browse:get-raw-data($coll as xs:string?){
    if($coll = 'places') then concat("collection('",$global:data-root,"/places/tei')//tei:place",browse:get-syr(),facets:facet-filter())
    else if(exists($coll)) then concat("collection('",$global:data-root,xs:anyURI($coll),"')//tei:body",browse:get-syr(),facets:facet-filter())
...
let $browse-path :=  browse:get-raw-data($coll)
return 
...
<div id="map-filter">
    <div class="collapse dropSheet map-filter" id="showFilters">
        {
        let $facets := $model("browse-data")//tei:location
        return facets:facets($facets)
        }
    </div>
    <a class="toggle-filter btn btn-default pull-right" data-toggle="collapse" data-target="#showFilters" data-text-swap="Hide Filter">Filter Places</a>
</div>
};
